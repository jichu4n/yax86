...
// A power of two, as CPUSetDecodeCache() requires, and large enough that
// nothing these tests run collides.
...
// A tick driven by PlatformRun() runs several instructions back to back where
// nothing needs to see the boundaries between them. These pin down what ends a
// run, since every one of them is a point at which the emulation would
// otherwise diverge - an interrupt delivered late, a device serviced late, a
// trap not taken.
//
// Everything here drives CPUTick() directly with a budget set by hand, which
// is what PlatformRun() does for the platform. Runs are only ever entered from
// the decode cache, so each test warms it first.
class InstructionsPerTickTest : public ::testing::Test {
 protected:
  ...
  // where a run takes its second instruction and every one after it from. A
  // budget of zero runs one instruction per tick however warm the cache gets.
  ...
  // Runs one tick with a budget large enough not to be what ends the run, and
...
// code before runs one instruction per tick however large its budget. This is
// what keeps the 8088 hardware suite a single-instruction test while it
// supplies a budget.
...
TEST_F(InstructionsPerTickTest, NoDecodeCacheRunsOneInstructionPerTick) {
  Load(kProgramAddress, {kOpIncAx, kOpIncAx, kOpIncAx, kOpHlt});
  WarmCache(3);
  // Taking the cache away is what the platform does while a memory watchpoint
  // is enabled.
...
// The budget is what a host that has not asked for runs leaves at zero, and it
// is the first thing the continuation test looks at.
...
TEST_F(InstructionsPerTickTest, ARunStopsWhenAStopIsRequested) {
  ...
  cpu_.config.on_after_execute_instruction = [](CPUState* cpu, const Instruction*) {
