...
// one go will do; what matters is that it is more than one.
constexpr uint16_t kNumProgramInstructions = 8;

// PlatformRun() lets a tick run several instructions back to back, which
// PlatformTick() does not. These pin down both halves of that, and the two
// debug features that have to take it away again.
...
TEST_F(PlatformExecutionTest, PlatformRunBatchesCachedInstructions) {
  ...
  // Every one of them is cached and none needs the host's attention, so a
  // single tick runs the lot - which holds only while the program is shorter
  // than a run may be.
...
// A breakpoint is tested before a tick, so a tick that ran several
// instructions would run straight past one in the middle of a run.
