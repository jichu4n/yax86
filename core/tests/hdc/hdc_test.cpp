...
TEST_F(HDCOptionROMTest, LooksLikeAnOptionROM) {
  ...
  EXPECT_EQ(HDCReadOptionROMByte(0), kOptionROMSignature0);
  EXPECT_EQ(HDCReadOptionROMByte(1), kOptionROMSignature1);

  // Third byte is the ROM's size in 512-byte blocks, and it has to agree with
  // how much data is actually here.
  EXPECT_EQ(
      HDCReadOptionROMByte(2) * kOptionROMBlockSize, HDCGetOptionROMSize());
}
...
TEST_F(HDCOptionROMTest, ChecksumIsValid) {
  ...
  for (uint32_t i = 0; i < HDCGetOptionROMSize(); ++i) {
    sum = (uint8_t)(sum + HDCReadOptionROMByte(i));
  }
  EXPECT_EQ(sum, 0);
}
...
TEST_F(HDCOptionROMTest, ReadsPastTheEndReturnOpenBus) {
  EXPECT_EQ(HDCReadOptionROMByte(HDCGetOptionROMSize()), kHDCOpenBusValue);
  EXPECT_EQ(HDCReadOptionROMByte(0xFFFFFFFF), kHDCOpenBusValue);
}
