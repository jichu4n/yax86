...
### cpu — runs of several instructions per tick
...
- A tick driven through `PlatformRun()` executes up to `kMaxInstructionsPerTick`
  ...
  **8.42% at `-O3`** and 10.45% at `-O2`, which is the largest win left after
...
- **Breakpoints and step mode are suppressed from the platform, not the CPU.**
  ...
  `can_run_several_instructions_per_tick` from them and `PlatformTick()`
...
| ----- | ------- | ------------ | ---- | -------------- | ----------- | ---------- | ------------ |
| `-O3` | **4.774840** | **5.802** | **0.488** | **121.6%** | 473,868 | 182,200 | 89,455 |
| `-O2` | 5.106659 | 5.425 | 0.456 | 113.7% | 460,668 | 175,976 | 75,131 |
