...
TEST_F(StringRepCmpTest, SCASBBasic) {
    ...
    helper->cpu_.registers[kAX] = (helper->cpu_.registers[kAX] & 0xFF00) | 0x55; // Set AL = 0x55
...
TEST_F(StringRepCmpTest, SCASBNotEqual) {
    ...
    auto helper = CPUTestHelper::CreateWithProgram("scasb-not-equal-test", "scasb\n");
    helper->cpu_.registers[kES] = 0x030;
    helper->cpu_.registers[kDI] = 0x00;
    helper->cpu_.registers[kAX] = (helper->cpu_.registers[kAX] & 0xFF00) | 0x33; // Set AL = 0x33
...
TEST_F(StringRepCmpTest, SCASBBackward) {
    ...
    auto helper = CPUTestHelper::CreateWithProgram("scasb-backward-test", "scasb\n");
    helper->cpu_.registers[kES] = 0x050;
    helper->cpu_.registers[kDI] = 0x00;
    helper->cpu_.registers[kAX] = (helper->cpu_.registers[kAX] & 0xFF00) | 0x77; // Set AL = 0x77
...
TEST_F(StringRepCmpTest, SCASWBackward) {
    ...
    auto helper = CPUTestHelper::CreateWithProgram("scasw-backward-test", "scasw\n");
...
TEST_F(StringRepCmpTest, REPEScasbFound) {
    ...
    auto helper = CPUTestHelper::CreateWithProgram("repe-scasb-found-test", "repe scasb\n");
    helper->cpu_.registers[kES] = 0x070;
    helper->cpu_.registers[kDI] = 0x00;
    helper->cpu_.registers[kAX] = (helper->cpu_.registers[kAX] & 0xFF00) | 0xAA; // Set AL = 0xAA
    ...
    EXPECT_EQ(helper->cpu_.registers[kDI], 0x03);  // Points after the unequal byte
    EXPECT_EQ(helper->cpu_.registers[kCX], 1);     // One iteration left
    EXPECT_FALSE(GetFlag(&helper->cpu_, kZF));     // ZF clear because last comparison failed
}

TEST_F(StringRepCmpTest, REPEScasbNotFound) {
    // Test REPE SCASB - all bytes equal, CX reaches zero
    auto helper = CPUTestHelper::CreateWithProgram("repe-scasb-not-found-test", "repe scasb\n");
    helper->cpu_.registers[kES] = 0x080;
    helper->cpu_.registers[kDI] = 0x00;
    helper->cpu_.registers[kAX] = (helper->cpu_.registers[kAX] & 0xFF00) | 0xCC; // Set AL = 0xCC
    ...
    EXPECT_TRUE(GetFlag(&helper->cpu_, kZF));      // ZF set because last comparison succeeded
}

TEST_F(StringRepCmpTest, REPEScaswFound) {
    // Test REPE SCASW - find unequal word
    auto helper = CPUTestHelper::CreateWithProgram("repe-scasw-found-test", "repe scasw\n");
    ...
    helper->memory_[0x902] = 0x22;  // Second word doesn't match - should stop here
    ...
    EXPECT_EQ(helper->cpu_.registers[kDI], 0x04);  // Points after the unequal word
    EXPECT_EQ(helper->cpu_.registers[kCX], 1);     // One iteration left
    EXPECT_FALSE(GetFlag(&helper->cpu_, kZF));     // ZF clear because last comparison failed
}
...
TEST_F(StringRepCmpTest, REPNEScasbFound) {
    ...
    auto helper = CPUTestHelper::CreateWithProgram("repne-scasb-found-test", "repne scasb\n");
    helper->cpu_.registers[kES] = 0x0A0;
    helper->cpu_.registers[kDI] = 0x00;
    helper->cpu_.registers[kAX] = (helper->cpu_.registers[kAX] & 0xFF00) | 0xDD; // Set AL = 0xDD
...
TEST_F(StringRepCmpTest, REPNEScasbNotFound) {
    ...
    auto helper = CPUTestHelper::CreateWithProgram("repne-scasb-not-found-test", "repne scasb\n");
    helper->cpu_.registers[kES] = 0x0B0;
    helper->cpu_.registers[kDI] = 0x00;
    helper->cpu_.registers[kAX] = (helper->cpu_.registers[kAX] & 0xFF00) | 0xFF; // Set AL = 0xFF
    helper->cpu_.registers[kCX] = 3;  // Check 3 bytes
...
TEST_F(StringRepCmpTest, REPNEScaswFound) {
    ...
    auto helper = CPUTestHelper::CreateWithProgram("repne-scasw-found-test", "repne scasw\n");
...
TEST_F(StringRepCmpTest, SCASBZeroCount) {
    ...
    auto helper = CPUTestHelper::CreateWithProgram("scasb-zero-count-test", "repe scasb\n");
    helper->cpu_.registers[kES] = 0x0D0;
    helper->cpu_.registers[kDI] = 0x00;
    helper->cpu_.registers[kAX] = (helper->cpu_.registers[kAX] & 0xFF00) | 0x99; // Set AL = 0x99
...
TEST_F(StringRepCmpTest, SCASBSignedComparison) {
    ...
    auto helper = CPUTestHelper::CreateWithProgram("scasb-signed-test", "scasb\n");
    helper->cpu_.registers[kES] = 0x0E0;
    helper->cpu_.registers[kDI] = 0x00;
    helper->cpu_.registers[kAX] = (helper->cpu_.registers[kAX] & 0xFF00) | 0x7F; // Set AL = 0x7F (+127 in signed interpretation)
    ...
    EXPECT_TRUE(GetFlag(&helper->cpu_, kCF));   // 0x7F < 0x80 in unsigned comparison
    EXPECT_TRUE(GetFlag(&helper->cpu_, kSF));   // Sign flag reflects result sign
    EXPECT_TRUE(GetFlag(&helper->cpu_, kOF));   // Overflow: positive - negative = overflow
}

TEST_F(StringRepCmpTest, REPEScasbBackward) {
    // Test REPE SCASB in backward direction
    auto helper = CPUTestHelper::CreateWithProgram("repe-scasb-backward-test", "repe scasb\n");
    ...
    EXPECT_EQ(helper->cpu_.registers[kDI], 0xFFFF); // Points to 0xF00-1 (wrapped)
    EXPECT_EQ(helper->cpu_.registers[kCX], 0);       // All iterations completed
    EXPECT_FALSE(GetFlag(&helper->cpu_, kZF));       // ZF clear because last comparison failed
}