...
typedef struct {
  ...
  // Raw immediate data bytes. If immediate_size is 1, only imm_bytes[0] is
  // valid. If immediate_size is 2, imm_bytes[0] is the low byte, imm_bytes[1]
  // is the high byte.
  uint8_t immediate[2];
  ...
  // Number of immediate data bytes present: 0, 1, or 2.
  uint8_t immediate_size : 2;

  // Total length of the original encoded instruction in bytes (1 to 6).
  uint8_t size;
} EncodedInstruction;
...
typedef struct {
  ...
  bool has_modrm : 1;
  // Number of immediate data bytes: 0, 1, or 2
  uint8_t immediate_size : 2;
} OpcodeMetadata;
