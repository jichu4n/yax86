...
static uint8_t SDLToXTScancode(SDL_Keycode key) {
  switch (key) {
    ...
    case SDLK_APOSTROPHE: return 0x28;
    case SDLK_GRAVE: return 0x29;
    ...
    // Extended keys (using standard scancodes, often prefixed by E0 in real hardware
    // but here passing simplified make codes if supported or just basic ones)
    // For simplicity, mapping to primary block or similar if appropriate.
    // Up/Down/Left/Right are often E0 prefixed. 
    // XT standard:
    // Up: 48 (same as KP 8)
    // Left: 4B (same as KP 4)
    // Right: 4D (same as KP 6)
    // Down: 50 (same as KP 2)
