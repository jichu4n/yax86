...
# Summary
# -----------------------------------------------------------------------------
# Flash and SRAM are what has to fit on the board, and agree with what the
# linker printed during the link. The core's own text is the figure to line up
# against a desktop measurement of the same object, since it leaves out the SDK
# and the harness. image-size.js reads all three out of the ELF - see the note
# at the top of it for why it does that rather than parse readelf and size.
echo
printf '%-6s %10s %10s %10s  %s\n' opt flash sram core_text uf2
for opt in "${OPT_LEVELS[@]}"; do
    build="$BUILD_ROOT/$opt"
    # The object is spelled out rather than searched for: CMake names an object
    # after the absolute path of its source, so a build directory reused across
    # two YAX86_CORE_ROOTs holds one object per core, and a search would report
    # whichever it happened to find first.
    core_obj="$build/CMakeFiles/yax86_pico_bench.dir/${CORE_ROOT#/}/core/yax86_core.c.o"
    read -r flash sram core_text < <(
        ./image-size.js "$build/yax86_pico_bench.elf" "$core_obj")
    printf '%-6s %10s %10s %10s  %s\n' \
        "-$opt" "$flash" "$sram" "$core_text" \
        "$build/yax86_pico_bench.uf2"
done
